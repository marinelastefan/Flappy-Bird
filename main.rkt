...
(define-struct state-struct (bird-struct variables-struct pipes scor))
(define initial_bird (bird-struct bird-x bird-initial-y 0))
(define initial_variables (variables-struct initial-gravity initial-momentum initial-scroll-speed))
...
;(define p1 (pipe-struct scene-width(+ added-number (random random-threshold)) 0))
;(define (add-the-first-pipe pipes )
  ;(append (list (pipe-struct scene-width(+ added-number (random random-threshold)) 0) ) pipes))
...
(define (get-initial-state)
  (state-struct initial_bird
                initial_variables
                (append (list (pipe-struct scene-width (+ added-number (random random-threshold)))) '())
                 0))
...
(define (get-bird state)
 (state-struct-bird-struct state))
...
(define (next-state-bird bird gravity)
  (match-let ([(bird-struct x y vy) bird])
    (struct-copy bird-struct bird [y (+ y vy)] [vy (+ vy gravity)])))
...
(define (next-state-bird-onspace bird momentum)
  (match-let ([(bird-struct x y vy) bird]) (struct-copy bird-struct bird [vy (- 0 momentum)])))
...
(define (change current-state pressed-key)
  (match-let ([(state-struct bird-struct _ _ _) current-state])
    (if (key=? pressed-key " ")
         (struct-copy state-struct current-state [bird-struct (next-state-bird-onspace bird-struct initial-momentum)])
        current-state)))
...
(define (move-pipes pipes scroll-speed)
  (map (λ(pipe) (match-let ([(pipe-struct x _ ) pipe ]) (struct-copy pipe-struct pipe [x (- x scroll-speed)]))) pipes)) 
...
(define (clean-pipes pipes)
  (filter (λ(pipe) (match-let ([(pipe-struct x _ ) pipe]) (< 0 (+ x pipe-width)))) pipes))
...
; cu pasărea.
 (define (add-more-pipes pipes)
  (if (< (length pipes) no-pipes)
      ...
      (append pipes (list (pipe-struct (+ pipe-width pipe-gap (get-pipe-x (last pipes)))
...
(define (check-ground-collision bird)
 (match-let ([(bird-struct _ y _) bird])
   (let ((ybird (+ y bird-height)))
   (if (or (> ybird ground-y) (equal? ybird ground-y)) #t
       #f))))
...
(define (invalid-state? state)
  (match-let ([(state-struct bird-struct _ pipes _) state])
     (or (check-ground-collision bird-struct)
            (check-pipe-collisions bird-struct pipes))))
...
(define (check-pipe-collisions bird pipes)
  (if (null? pipes) #f
      (match-let ([(pipe-struct x y ) (car pipes)])
        (if (or (equal? #t (check-collision-rectangles
                     (make-posn (get-bird-x bird) (get-bird-y bird))
                     (make-posn (+ (get-bird-x bird) bird-width) (+ (get-bird-y bird) bird-height))
                     (make-posn x 0)
                     (make-posn (+ x pipe-width) y)))
                (equal? #t (check-collision-rectangles
                            (make-posn (get-bird-x bird) (get-bird-y bird))
                            (make-posn (+ (get-bird-x bird) bird-width) (+ (get-bird-y bird) bird-height))
                            (make-posn x (+ y pipe-self-gap))
                            (make-posn (+ x pipe-width) (+ y pipe-self-gap pipe-height))))) 
            #t
...
(define (next-state state)
  (match-let ([(state-struct bird-struct variables-struct  pipes scor)  state])
    (struct-copy state-struct state [bird-struct (next-state-bird (get-bird state) initial-gravity)]
...
(define (draw-frame state)
  (match-let ([(state-struct bird-struct  _ pipes  scor) state])
      ;bird
     (place-image bird-image (+ (get-bird-x bird-struct) ( quotient bird-width 2)) (+ (get-bird-y bird-struct) (quotient bird-height 2)) 
      ;ground
     (place-image ground-image (quotient scene-width 2) ( + (quotient ground-height 2) ground-y) 
      ;scor
     (place-image (score-to-image scor) (posn-x text-posn) (posn-y text-posn) 
       ;pipes
...
(define (place-pipes pipes scene)
  (foldl (λ(pipe scene) (match-let ([(pipe-struct x y ) pipe])
                       (place-image gap-image (+ x  (quotient pipe-width 2)) (+ y (quotient pipe-self-gap 2)) 
                       (place-image pipe-image (+ x (quotient pipe-width 2))  (quotient pipe-height 2) scene)))) scene pipes))
